...
# ── WINDSOR ───────────────────────────────────────────────────────────────────
# Uses the same /all endpoint Windsor MCP uses internally, with JSON-encoded
# accounts and fields arrays — exactly matching what succeeds via MCP.
def windsor_get(fields, date_from, date_to, extra_filter=None):
    params = {
        "api_key":    WINDSOR_API_KEY,
        "connector":  "google_ads",
        "accounts":   json.dumps([GA_ACCOUNT]),
    ...
    url = "https://connectors.windsor.ai/all?" + urllib.parse.urlencode(params)
    print(f"  → Windsor [{date_from}→{date_to}] {fields[:3]}{'…' if len(fields)>3 else ''}", flush=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
print("=== WINDSOR: Campaign metrics (yesterday) ===", flush=True)
...
print("=== WINDSOR: Campaign conversions (yesterday) ===", flush=True)
raw_conv = windsor_get(
    ["campaign","conversion_action_name","conversions"],
    YEST, YEST
)

print("=== WINDSOR: Daily 7d ===", flush=True)
raw_d7 = windsor_get(
...
print("=== WINDSOR: 14d + 30d aggregates ===", flush=True)
raw_14  = windsor_get(["impressions","clicks","cost","roas"], AGO14, YEST)
raw_14c = windsor_get(["conversion_action_name","conversions"], AGO14, YEST)
raw_30  = windsor_get(["impressions","clicks","cost","roas"], AGO30, YEST)
raw_30c = windsor_get(["conversion_action_name","conversions"], AGO30, YEST)
...
for r in raw_camp:
    ...
    camp_map[name] = {
        ...
        "roas":   safe_float(r.get("roas")),   # Windsor returns actual value (e.g. 5.6 = 560%)
        "purch":  0.0,
    }
for r in raw_conv:
    name = r.get("campaign","")
...
def agg_rows(rows):
    i=cl=co=rs=rn=0.0
    for r in rows:
        i+=safe_float(r.get("impressions")); cl+=safe_float(r.get("clicks"))
        co+=safe_float(r.get("cost")); rv=safe_float(r.get("roas",0))
        if rv: rs+=rv; rn+=1
    return {"impr":i,"clicks":cl,"cost":co,"roas":rs/rn if rn else 0}

agg14 = {**agg_rows(raw_14), "purch": agg_conv(raw_14c)}
agg30 = {**agg_rows(raw_30), "purch": agg_conv(raw_30c)}

# ═══════════════════════════════════════════════════════════════════════════════
print("=== POSTHOG: Daily funnel 7d (Google Paid) ===", flush=True)
...
print("=== POSTHOG: Channel funnel 30d ===", flush=True)
...
print(f"\n✅ data.json written — {len(campaigns)} campaigns, "
      f"{len(daily7)} daily, {len(channels)} channels, {len(daily14)} daily14", flush=True)
